...
public class UserUI : MonoBehaviour {
    ...
    private void OnGUI() {
        ...
        if (flag) {
            ...
            if (GUI.Button(new Rect(Screen.width/2-190, Screen.height/2 -30, 185, 70), "Physics Mode", style1)) {
                action.setMode(ActionMode.PHYSICS);
                flag = false;
                action.changeState(1);
            }
            if (GUI.Button(new Rect(Screen.width/2+10, Screen.height/2 -30, 190, 70), "Dynamics Mode", style1)) {
